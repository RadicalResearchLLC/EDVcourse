...
First let's use the default palette for **BIR79** to show the county birthrates in 1979. Adding `fill = BIR79` to the `aes()` defaults to the _Blues_ palette. @fig-ggFill shows the result of adding a fill color scale. 
...
```
Let's change that to a viridis color scale. The function `scale_fill_viridis_c()` adds a fancier color-blind viridis palette in a continuous scale.  
...
The `nc` dataset has a variable called _NAME_ for the county names. @fig-ggNames shows the figure when we add the
...
@fig-ggSoCal shows a potential example of what that might look like.

```{r}

#| label: fig-ggSoCal
#| fig-cap: ggplot map for SoCal 
#| echo: false
