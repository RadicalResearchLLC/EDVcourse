...
[NOAA Global Monitoring Laboratory](https://gml.noaa.gov/) provides text file data that we can directly access on greenhouse gases.   
...
Hmmm, it looked promising but nope. It assigned everything to one column. Those aren't tabs delimiting the columns. I will try `read_table()` instead since that covers whitespace-separated values.
...
Nope, the column names are going to need to be manually assigned. The `colnames()` function allows a user to rename column headers. 
...
Looking good!  Let's take quick look with a visualization (sorry, can't resist). I do need to do one processing step to combine the month and year columns. I'll do that the quick and dirty way rather than deal with _date_ formats which are notoriously problematic.
...
Dealing with other people's data is fraught with peril. Data is hard to deal with. I include this example to indicate that dealing with data import and tidying is 80+% of being able to visualize a dataset.
:::

### Downloading a well-behaved dataset

Let's go grab the CalEnviroScreen Shapefile. I promise you this dataset is better behaved.
...
We want that shapefile, which is zipped again. I assign the path, then save it as a `.zip` file. On my Windows machine download.file didn't work, which is a known certificate issue on https:// type downloads. If it doesn't work 
...
```{r}
#| label: alternate download option
#| echo: true
#| 
...
```{r}
#| label: unzip and read the shapefile data 1
#| echo: true

unzipped_EJ <- unzip('CalEJ.zip')
#read_sf(unzipped_EJ)
