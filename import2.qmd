# Importing Data - Part 2 {#sec-import2}
...
1.  Identify the directory where the zipped shapefile was downloaded. On my machine, this is a **Downloads** folder which can be accessed through my web browser after the file download is complete as shown in @fig-openFolder. The name of the file is **calenviroscreen40shpf2021shp.zip**.

![Browser download](/images/download_complete.png){#fig-openFolder} 2. Identify the R working directory on your machine using the `getwd()` function.
...
Check the *Files* panel. Is there a new **CalEJ4** folder as shown in @fig-codeUnzip?

![Another Shapefile folder is in the working directory!](/images/double.png){#fig-codeUnzip} \### Import the Shapefile
...
4. An **Actions** button should appear as shown on @fig-Action. Press the Actions button, select _Export All_ and choose **Export to geoJSON**.
...
Let's look at in ggplot.

```{r}
#| label: ggplot map of CA EJI
#| echo: true
