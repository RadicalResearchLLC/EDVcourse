# Introduction to Spatial Visualization {#sec-maps1}
...
Lastly, we assign the North Carolina data we're reading into a dataset using the `<-` operator. The `<-` operator tells R that the data we are loading should be placed into a dataset that we have named `nc`. In future functions, `nc` will access that underlying data within `nc`.
...
Next, we apply the same `st_read()` function to read the geoJSON file. geoJSON is a common format for encoding geographic data structures, and `st_read()` natively knows how to read it in. There's also a reference coordinate transformation using `st_transform()` required to display this properly in `leaflet` in the [WGS84](https://gisgeography.com/wgs84-world-geodetic-system/) coordinate system. 
