...
0. **Step 0** - [Install R and RStudio](http://radicalresearch.llc/EDVcourse/tools.html) - should have this done already
...
#### 4A. Choose the visualization type
...
#### 4B. Choose the visualization function
...
#### 4C. Write the code to do a _basic_ visualization
...
@fig-basic shows a very basic example using `mpg` as our dataset, `ggplot` as our first action (i.e., make a figure), and `geom_point` as our category of visualization (with points).
...
This is our most basic point plot.  The x-axis shows engine displacement (Liters) for gasoline vehicles, and the y-axis shows highway driving fuel efficiency in miles per gallon.

Three functions were used.  

`ggplot()` - make a figure
...
```
@fig-line shows a basic line plot using `geom_line` instead of points. It is a visual abomination.
...
The last new thing I want to show is a `facet_wrap()` which will make this visualization much easier to interpret on a class basis. @fig-face shows how this works on our basic visualization by adding a line to our basic visualization.  
