...
```{r}
#| label: import NFA dataset and clean its column names
#| echo: true
...
names(country_results)

country_results2 <- clean_names(country_results)
names(country_results2)

```

The names are now all lower case, and there are no spaces or `...` or parentheses. There are extra arguments to provide different naming conventions for capitalization (snake, lower_camel, upper_lower, etc.)
...
Here are three examples using SoCalEJ. First, example was done to remove the values that were set at -999; we only believed values from 0-100 were reasonable. @fig-AsthmaCounty uses filter to remove those values. I've also shown an example without the filter applied. Not removing those rows messes up our visualization.   

```{r}
#| label: fig-AsthmaCounty
